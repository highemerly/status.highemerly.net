...
`h` は履歴で、**1 文字が 1 点（5 分）**を表す。各点はその時刻で終わる 5 分間を指す。
...
Lambda は Prometheus を 5 分ではなく **`SUB_STEP_SECONDS`（既定 60 秒）刻みで読み**、
5 分ぶんを 1 文字に畳む。`query_range` は範囲を集計せず評価時刻ごとの直近 1 サンプルを
返すだけなので、5 分刻みで撃つと 5 分に 1 点しか見ず、途中の失敗が消える。
`0` になるのは 5 分続けて落ちていたときだけで、単発の失敗は `d` として残る。

各点にタイムスタンプを持たせず `from` / `step` から逆算するため、
48 時間分を持っても 7.5KB に収まる（素朴な形式なら 819KB）。
細かく読んでも畳んだあとの点数は変わらないので、サイズには影響しない。
